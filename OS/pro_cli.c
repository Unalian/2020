...
void *producer_func(void *param) {
    ...
    for (;;) {
        ...
        sem_wait(&empty);//wait信号量
        pthread_mutex_lock(&mutex);//互斥锁
        if (input_point >= strlen(input_data)) {//如果指针超过边界，即报告后退出
            printf("producer: %d produce nothing and ready to exit.\n", number);
            pthread_mutex_unlock(&mutex);
            sem_post(&empty);
            break;
        }
        buf[tail] = input_data[input_point];//加入buffer
        printf("producer: %d produce the character: %c\n", number, buf[tail]);
        input_point = input_point + 1;//指针后移
        tail = (tail + 1) % N;//指针后移
        sem_post(&full);//post信号量
        if (input_point >= strlen(input_data)) {//此时，当input_point超过界限，打印
            printf("producer: %d produce nothing and ready to exit.\n", number);
            pthread_mutex_unlock(&mutex);
            sem_post(&empty);
            break;
